...
const log = createLogger("reconcile/pdf-extract");
...
/**
 * Main extraction pipeline. Tries three strategies in order:
 * 1. Direct text parsing (instant, free)
 * 2. Text + AI (fast, 2-5s)
 * 3. Vision AI fallback (for scanned PDFs)
 */
export async function extractRowsFromPdf(buffer: Buffer): Promise<ExtractedRow[]> {
  let pdfText = "";
  try {
    const { text } = await extractText(new Uint8Array(buffer), { mergePages: true });
    pdfText = typeof text === "string" ? text : "";
    log.debug(`Text extracted: ${pdfText.length} chars`);
  } catch (e) {
  ...
  if (pdfText.length > 50) {
    ...
    if (directRows.length > 0) {
      log.debug(`Direct parse succeeded: ${directRows.length} rows`);
      return directRows;
    }

    log.debug("Direct parse got 0, trying text AI...");
    const aiRows = await parseTextWithAI(pdfText);
    if (aiRows.length > 0) {
      log.debug(`Text AI succeeded: ${aiRows.length} rows`);
      return aiRows;
    }
  }

  log.debug("Text approaches failed, trying vision AI...");
  const visionRows = await parseWithVisionAI(buffer);
  if (visionRows.length > 0) {
    log.debug(`Vision AI succeeded: ${visionRows.length} rows`);
    return visionRows;
  }
...
// ---------------------------------------------------------------------------
// Strategy 1: Direct text parsing
// ---------------------------------------------------------------------------
...
function parseTextDirect(text: string): ExtractedRow[] {
  ...
  if (isPP36) {
    log.debug("Detected PP36 format");
    ...
    return raw
      ...
      .map((item) => {
        ...
        return {
          ...
          taxId: toStr(item.taxId ?? item.tax_id ?? "").replace(/[^a-zA-Z0-9\-]/g, ""),
          baseAmount: base,
          vatAmount: vat,
