 ...
 * อนุมัติคำขอเบิกจ่าย (สำหรับ Manager/Admin)
 * 
...
export async function POST(request: Request, { params }: RouteParams) {
  return withAuth(async (req, { session }) => {
    ...
    if (
      ...
      return apiResponse.badRequest("Can only approve PENDING or FLAGGED reimbursements");
    }
    ...
    if (!canApproveReimbursement && !canApproveExpense) {
      return apiResponse.forbidden("คุณไม่มีสิทธิ์ในการอนุมัติคำขอเบิกจ่าย");
    }

    // Cannot approve own request
    if (expense.requesterId === session.user.id) {
      return apiResponse.badRequest("ไม่สามารถอนุมัติคำขอเบิกจ่ายของตัวเองได้");
    }
    ...
    await createAuditLog({
      ...
      entityType: "Expense",
      entityId: id,
      description: `อนุมัติเบิกจ่าย: ${expense.description || "ไม่ระบุ"} จำนวน ${expense.netPaid} บาท`,
      changes: {
        before: { reimbursementStatus: expense.reimbursementStatus },
        after: { reimbursementStatus: "APPROVED" },
      },
    });
