...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (!access || !["OWNER", "MANAGER"].includes(access.role)) {
      return NextResponse.json(
        { error: "Only owners and managers can update LINE configuration" },
        { status: 403 }
      );
    }

    const body = await request.json();
    const { channelSecret, channelAccessToken, groupId } = body;
...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (!access || !["OWNER", "MANAGER"].includes(access.role)) {
      return NextResponse.json(
        { error: "Only owners and managers can remove LINE configuration" },
        { status: 403 }
      );
    }
